...
本 README 保留原版主要说明，并补充 v1.5.3 本次注册机修复、部署方式和使用注意事项。
...
- 注册守护 watchdog 增加 fd 超阈值自动停启保护，降低长期运行后文件描述符耗尽风险。
...
v1.5.3 修复“可用账号数量模式达到目标后持续检查，号池实际减少但注册页正常账号数量不再同步，检查日志停止”的问题。
...
- watchdog 会监控进程 fd 数量，超过阈值时自动停止注册任务，等待守护线程退出、回收资源后再重新启动。
