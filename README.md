...
<p align="center">基于 <a href="https://github.com/basketikun/chatgpt2api">basketikun/chatgpt2api</a> v1.5.0 的二开分支，当前版本 v1.5.1。本次更新聚焦注册机长期运行稳定性，修复可用账号模式下号池数量不同步和守护线程静默停止问题。</p>
...
本 README 保留原版主要说明，并补充 v1.5.1 本次注册机修复、部署方式和使用注意事项。

## v1.5.1 更新日志

本次更新只记录 v1.5.1 当前修复：

- 修复注册机可用账号模式长时间运行后“正常账号”显示不同步的问题，注册页状态会实时读取号池数量。
- 为注册守护线程增加异常自愈，检查号池、保存状态或自动清理代理黑名单异常时会写入实时日志，并按检查间隔继续重试。
- 状态返回新增 `runner_alive`；任务仍启用但守护线程已停止时，会在下一次状态读取时自动重启。
- 将代理黑名单清理改为按代理刷新间隔自动执行，复用原有手动清理逻辑，不改变黑名单评分和冷却规则。
